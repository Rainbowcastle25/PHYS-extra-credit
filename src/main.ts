...
function renderStudyGuide(): void {
  const container = getById<HTMLDivElement>('study-guide-list');
  ...
  container.querySelectorAll<HTMLButtonElement>('button[data-checkpoint]').forEach((button) => {
    button.addEventListener('click', () => {
      const topicId = button.dataset.checkpoint;
      if (!topicId) {
        return;
      }
      const answer = getById<HTMLParagraphElement>(`checkpoint-${topicId}`);
      answer.classList.toggle('hidden');
    });
  });
}

function renderFormulaSheet(): void {
  const container = getById<HTMLDivElement>('formula-sheet-list');
      ...
      <article class="formula-card">
        ...
        <ul>
          ${formulas
            .map(
              (formula) =>
                `<li><strong>${formula.name}</strong><br /><code>${formula.expression}</code><br /><span>${formula.note}</span></li>`
            )
            .join('')}
        </ul>
      </article>
...
function setupPracticeExam(): void {
  const modeSelect = getById<HTMLSelectElement>('exam-mode');
  const topicSelect = getById<HTMLSelectElement>('exam-topic');
  const countInput = getById<HTMLInputElement>('exam-count');
  const timedSelect = getById<HTMLSelectElement>('exam-timed');
  const minutesInput = getById<HTMLInputElement>('exam-minutes');
  const startButton = getById<HTMLButtonElement>('start-exam');
  const status = getById<HTMLParagraphElement>('exam-status');
  const examArea = getById<HTMLDivElement>('exam-area');
  ...
  let activeExam: ActiveExam | null = null;
  ...
  const renderQuestion = (): void => {
    ...
    examArea.querySelectorAll<HTMLInputElement>('input[name="answer"]').forEach((input) => {
    ...
    getById<HTMLButtonElement>('exam-prev').addEventListener('click', () => {
      if (!activeExam) {
        return;
      }
    ...
    getById<HTMLButtonElement>('exam-next').addEventListener('click', () => {
      if (!activeExam) {
        return;
      }
  ...
  const renderHistory = (): void => {
    const historyArea = getById<HTMLDivElement>('history-area');
    const history = loadJson<PracticeAttempt[]>(storageKeys.practiceHistory, []);
    if (history.length === 0) {
  ...
  startButton.addEventListener('click', () => {
    ...
    activeExam = {
      questions,
      answers: Array.from<number | null>({ length: questions.length }).fill(null),
      currentIndex: 0,
      startedAt: Date.now(),
      timed,
...
function setupSimulators(): void {
  const projOutput = getById<HTMLParagraphElement>('proj-output');
  getById<HTMLButtonElement>('proj-run').addEventListener('click', () => {
  ...
  const forceOutput = getById<HTMLParagraphElement>('force-output');
  getById<HTMLButtonElement>('force-run').addEventListener('click', () => {
  ...
  const energyOutput = getById<HTMLParagraphElement>('energy-output');
  getById<HTMLButtonElement>('energy-run').addEventListener('click', () => {
  ...
  const momentumOutput = getById<HTMLParagraphElement>('mom-output');
  getById<HTMLButtonElement>('mom-run').addEventListener('click', () => {
    const m1 = getNumber('mom-m1');
    const u1 = getNumber('mom-u1');
    const m2 = getNumber('mom-m2');
    const u2 = getNumber('mom-u2');
    const type = getById<HTMLSelectElement>('mom-type').value;
  ...
  const modeSelect = getById<HTMLSelectElement>('osc-mode');
  const shmInputs = getById<HTMLDivElement>('osc-shm-inputs');
  const rotInputs = getById<HTMLDivElement>('osc-rot-inputs');
  const oscOutput = getById<HTMLParagraphElement>('osc-output');
  ...
  getById<HTMLButtonElement>('osc-run').addEventListener('click', () => {
...
function setupFlashcards(): void {
  const topicSelect = getById<HTMLSelectElement>('flash-topic');
  const cardBox = getById<HTMLElement>('flash-card');
  const meta = getById<HTMLParagraphElement>('flash-meta');
  const reviewedSet = new Set(loadJson<string[]>(storageKeys.reviewedFlashcards, []));
  ...
  getById<HTMLButtonElement>('flash-prev').addEventListener('click', () => {
    if (filtered.length === 0) {
      return;
    }
  ...
  getById<HTMLButtonElement>('flash-next').addEventListener('click', () => {
    if (filtered.length === 0) {
      return;
    }
  ...
  getById<HTMLButtonElement>('flash-random').addEventListener('click', () => {
    if (filtered.length === 0) {
      return;
    }
  ...
  getById<HTMLButtonElement>('flash-reveal').addEventListener('click', () => {
    if (filtered.length === 0) {
      return;
    }
    revealed = !revealed;
...
function setupChecklist(): void {
  const box = getById<HTMLDivElement>('checklist-box');
  const completed = new Set(loadJson<string[]>(storageKeys.checklist, []));

  box.innerHTML = checklistItems
    .map(
      (item) =>
        `<label><input type="checkbox" data-checklist="${item.id}" ${completed.has(item.id) ? 'checked' : ''}/> ${item.label}</label>`
    )
    .join('');

  box.querySelectorAll<HTMLInputElement>('input[data-checklist]').forEach((input) => {
    input.addEventListener('change', () => {
...
const conversions: Record<string, { label: string; convert: (value: number) => string }> = {
...
function setupConverter(): void {
  const typeSelect = getById<HTMLSelectElement>('convert-type');
  const valueInput = getById<HTMLInputElement>('convert-value');
  const output = getById<HTMLParagraphElement>('convert-output');
...
function setupFormulaFinder(): void {
  const select = getById<HTMLSelectElement>('finder-select');
  const output = getById<HTMLParagraphElement>('finder-output');
...
function setupTopicTracker(): void {
  const box = getById<HTMLDivElement>('topic-progress');
  const completed = new Set(loadJson<string[]>(storageKeys.masteredTopics, []));

  box.innerHTML = studyTopics
    .map(
      (topic) =>
        `<label><input type="checkbox" data-topic-track="${topic.id}" ${completed.has(topic.id) ? 'checked' : ''}/> ${topic.title}</label>`
    )
    .join('');

  box.querySelectorAll<HTMLInputElement>('input[data-topic-track]').forEach((input) => {
    input.addEventListener('change', () => {
...
function updateProgressSummary(): void {
  const summary = getById<HTMLDivElement>('progress-summary');
  const checklistDone = loadJson<string[]>(storageKeys.checklist, []).length;
  const topicsDone = loadJson<string[]>(storageKeys.masteredTopics, []).length;
  const cardsReviewed = loadJson<string[]>(storageKeys.reviewedFlashcards, []).length;
  const attempts = loadJson<PracticeAttempt[]>(storageKeys.practiceHistory, []);
...
function getNumber(id: string): number {
  const element = getById<HTMLInputElement>(id);
